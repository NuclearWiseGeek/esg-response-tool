...
def build_pdf(company_name, country, year, revenue, currency, df, totals, evidence_files, signer_name):
    ...
    # Wrapped in KeepTogether to ensure the signature never splits from the evidence
    closing_elements = []

    # A. EVIDENCE SECTION (Sorted by Scope 1 -> 2 -> 3)
    ...
    def add_footer(canvas, doc):
        ...
        # Line 1 (Top)
        canvas.drawCentredString(A4[0]/2, 42, "Generated by VSME Supplier ESG OS")
        
        # Line 2
        canvas.drawCentredString(A4[0]/2, 32, "Aligned with GHG Protocol & ISO 14064-1 quantification methodologies.")
        
        # Line 3
        canvas.drawCentredString(A4[0]/2, 22, "Supports CSRD ESRS E1 quantitative reporting requirements.")

        # Line 4 (Bottom - Source)
        canvas.drawCentredString(A4[0]/2, 12, "Emission Factors: ADEME Base Carbone v23.0 (France)")
